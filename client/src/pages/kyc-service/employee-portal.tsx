...
import { Shield, User, FileText, AlertCircle, CheckCircle2, Loader2, ArrowLeft } from "lucide-react";
...
export default function EmployeePortalPage() {
  ...
  const { data, isLoading, error } = useQuery<PortalData>({
    queryKey: ["/api/kyc-service/portal", slug, "employees"],
    enabled: !!slug,
  });

  const registerMutation = useMutation({
    mutationFn: async () => {
      const res = await apiRequest("POST", `/api/kyc-service/portal/${slug}/employees`, { name, email });
      return res.json();
    },
      ...
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <Card className="p-8 max-w-md w-full text-center space-y-4">
          ...
          <p className="text-muted-foreground" data-testid="text-error-message">
            This employee verification portal is not available or has been disabled.
          </p>
          <Link href="/">
          ...
          <div className="max-w-lg mx-auto">
            <Card className="p-8 text-center space-y-4">
              ...
              <p className="text-muted-foreground">
                A verification link has been sent to <strong>{email}</strong>. Please check your email to begin the verification process.
              </p>
      ...
      <main className="pt-20 pb-12 px-4">
        <div className="max-w-lg mx-auto space-y-6">
          <div className="text-center space-y-2">
            ...
            <h1 className="text-2xl font-bold" data-testid="text-portal-title">Employee Verification</h1>
            <p className="text-muted-foreground">
              <strong>{organisation.name}</strong> requires you to complete identity verification.
            </p>
          </div>
          ...
          <Card className="p-6 space-y-6">
            <div className="space-y-1">
              ...
              <p className="text-sm text-muted-foreground">
                Enter your details below to begin the verification process. You will receive an email with a verification link.
              </p>
            </div>
            ...
            >
              {registerMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <User className="h-4 w-4 mr-2" />}
