...
-   **Corporate Shareholders & Directors:** Full support for corporate entities as directors/shareholders across all application entry points, including specific KYB verification for corporate entities. Includes a three-path auto-verification cascade: (A) platform registry match via `verified_entities`, (B) authorised rep already a verified platform user, (C) live Smile ID CAC lookup (once only, gated by `kyb_lookup_status`). Supports both limited companies (RC) and business names (BN). Idempotency guards prevent duplicate Smile ID calls and duplicate records. Auto-verified entities show "Profile Ready / Platform Verified" badge in the UI. `company_people` schema extended with `corporate_business_type`, `kyb_lookup_status`, `kyb_lookup_attempted_at`, `auto_verify_method` fields.
...
-   **Lawyer Document Requests:** Lawyers can formally request specific documents from founders via a dedicated dialog in the application's Documents tab. Founders see open requests as alert banners in the Document Vault and can toggle "Share with Lawyer" per document file. Sharing auto-marks open requests as "actioned" (founder responded) and emails the lawyer. Lawyers see a "Documents Shared by Founder" panel listing all shared files with download links. Request creation + share/unshare actions are audit-logged. Schema: `lawyer_document_requests` table + `shareWithLawyer` boolean on `document_files`.
