...
-   **Existing Company Verified Onboarding:** A 5-step wizard (`/founder/existing-company`) allowing existing Nigerian companies to register on Cellion One. Includes: (1) CAC RC Lookup via Smile ID KYB (Job Type 7 / BUSINESS_REGISTRATION); (2) Company Detail Confirmation; (3) Director Management; (4) 6-document upload (CoI, MEMAT, CAC Status, TIN Cert, Proof of Address, Director ID); (5) Payment & Submission. Backend routes for KYB lookup, profile creation, document upload (Object Storage), submit-for-review, and admin approve/reject with email notifications. Admin panel at `/admin/existing-companies`. Dashboard banners for pending-review and rejected states. Status lifecycle: `draft → pending_payment → pending_review → documents_under_review → verified | rejected`. New SKUs: `EXISTING_CO_VERIFY` (₦25,000), `BANK_ACCOUNT` (manual pricing).
