...
-   **Lawyer Document Requests:** Lawyers can formally request specific documents from founders via a dedicated dialog in the application's Documents tab. Founders see open requests as alert banners in the Document Vault and can toggle "Share with Lawyer" per document file. Sharing auto-fulfils any open requests and emails the lawyer. Schema: `lawyer_document_requests` table + `shareWithLawyer` boolean on `document_files`.
