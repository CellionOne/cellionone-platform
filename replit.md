...
- **Landing Page:** World-class single-statement hero ("The Compliance Infrastructure for African Business") with Africa expansion positioning ("Starting in Nigeria. Building for Africa."), trust bar (4 key metrics), grouped desktop navigation (Products/Resources dropdowns, 4 items), mobile hamburger menu (Sheet slide-in), streamlined 7-section layout (hero → For Organisations → How It Works → Pricing → Services → FAQ → Contact → CTA), two-path KYC integration journey (Web Dashboard vs API), 3-column footer (Products/Company/Legal). Contact form with `POST /api/contact` (rate-limited, HTML-sanitized, sends to admin via Resend).
